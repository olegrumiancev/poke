...
document.addEventListener("DOMContentLoaded", () => {    
    ...
    for (const delay of delays) {    
      setTimeout(() => {    
        ...
        try {    
          ...
          if (isFinite(vt) && isFinite(at) && Math.abs(at - vt) > 0.40) {    
  ...
  const AUDIO_EVENT_SQUELCH_MS = 50;    
  ...
  function queuePostSeekRecovery() {    
    ...
    for (const delay of steps) {    
      const tid = setTimeout(async () => {    
        ...
        if (!isFinite(at) || Math.abs(at - vt) > 0.35) {    
          squelchAudioEvents(220);    
          safeSetCT(audio, vt);    
        }    
        ...
        if (isFinite(vt2) && (!isFinite(at2) || Math.abs(at2 - vt2) > 0.35)) {    
          squelchAudioEvents(220);    
          safeSetCT(audio, vt2);    
        }    
  ...
  function queueSeekResumeBurst() {    
    ...
    for (const delay of steps) {    
      const tid = setTimeout(async () => {    
        ...
        const vt = Number(video.currentTime());    
        if (isFinite(vt) && Math.abs(Number(audio.currentTime) - vt) > 0.35) safeSetCT(audio, vt);    
  ...
  async function seamlessBgCatchUp(reason = "resume") {    
    try {    
      ...
      const needVideoOnlyRepair =    
        ...
        !aPausedNow &&    
        (vPausedNow || (isFinite(vtNow) && isFinite(atNow) && Math.abs(atNow - vtNow) > 0.40));    
    ...
    try {    
      if (!seekSyncAudioSeeked) {    
        ...
        try {    
          ...
          if (!aSeeking2 && isFinite(vt1) && isFinite(at1) && Math.abs(at1 - vt1) < 0.40) {    
          ...
          if (intendedPlaying) {    
            ...
            if (shouldUseBgControllerRetry()) {    
              ...
              const needCatch =    
                ...
                isVideoPaused() ||    
                !!audio.paused ||    
                (isFinite(vt1) && isFinite(at1) && Math.abs(at1 - vt1) > 0.40);    
