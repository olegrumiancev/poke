...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (qua !== "medium" && hasExternalAudio) {
    ...
    const maybeStart = () => {
      ...
      startupBufferPrimed = true;document.addEventListener("DOMContentLoaded", () => {
  const video = videojs("video", {
    controls: true,
    autoplay: false,
    preload: "auto",
  ...
  function shouldIgnorePauseAsTransient() {
    ...
    if (mediaSessionForcedPauseActive()) return false;
    if (shouldTreatVisiblePauseAsUserPause()) return false;
    if (startupAutoplaySettlingActive() && intendedPlaying) return true;
  ...
  let startupAutoplayKickDone = false;
  let startupAutoplayKickInFlight = false;
  let startupAutoplaySettleUntil = 0;
  let startupAutoplayRetryTimer = null;
  let audioEverStarted = false;
  const startupPrimeStartedAt = performance.now();
  ...
  function startupAutoplaySettlingActive() {
    return wantsStartupAutoplay() && !startupAutoplayKickDone && performance.now() < startupAutoplaySettleUntil;
  }

  function armStartupAutoplaySettling(ms = 5000) {
    if (!wantsStartupAutoplay()) return;
    startupAutoplaySettleUntil = Math.max(
      startupAutoplaySettleUntil,
      performance.now() + Math.max(0, Number(ms) || 0)
    );
  }

  function startupAutoplaySatisfied() {
    if (!wantsStartupAutoplay()) return false;
    if (!hasExternalAudio || qua === "medium") return !isVideoPaused();
    return !isVideoPaused() && !audio.paused;
  }

  function finishStartupAutoplaySettling(success = false) {
    clearStartupAutoplayRetryTimer();
    if (success) startupAutoplayKickDone = true;
    startupAutoplaySettleUntil = 0;
  }

  function scheduleStartupAutoplayRetry(delay = 90) {
    if (!wantsStartupAutoplay()) return;
    if (startupAutoplayKickDone) return;
    if (!startupBufferPrimed) return;
    if (mediaSessionForcedPauseActive()) return;
    if (userPauseLockActive() || hardPauseLatchActive()) return;

    clearStartupAutoplayRetryTimer();
    startupAutoplayRetryTimer = setTimeout(async () => {
      startupAutoplayRetryTimer = null;
      ...
      armStartupAutoplaySettling(5000);
      ...
      strictBufferMissCount = 0;
      strictBufferHoldMinUntil = 0;
      strictBufferCoolDownUntil = Math.max(strictBufferCoolDownUntil, performance.now() + 250);
      ...
      if (!mediaSessionForcedPauseActive() && intendedPlaying && !isVideoPaused()) {
        queuePlayRetryBurst();
        await playTogether().catch(() => {});
      }

      if (startupAutoplaySatisfied()) {
        finishStartupAutoplaySettling(true);
      } else if (startupAutoplaySettlingActive() && intendedPlaying) {
        scheduleStartupAutoplayRetry(160);
      }
    }, Math.max(0, Number(delay) || 0));
  }

  function confirmStartupAutoplaySettled() {
    if (!wantsStartupAutoplay()) return;
    if (startupAutoplaySatisfied()) {
      finishStartupAutoplaySettling(true);
      return;
    ...
    if (
      startupAutoplaySettlingActive() &&
      intendedPlaying &&
      !mediaSessionForcedPauseActive() &&
      !userPauseLockActive() &&
      !hardPauseLatchActive()
    ) {
      scheduleStartupAutoplayRetry(100);
    }
  }
  ...
  function scheduleStartupAutoplayKick() {
    ...
    setTimeout(async () => {
      try {
        ...
        if (isVideoPaused()) {
          scheduleStartupAutoplayRetry(120);
          return;
        }
  ...
  async function playTogether() {
    ...
    try {
      ...
      if (!intendedPlaying && !isVideoPaused()) {
        ...
        if (startupAutoplaySettlingActive() && intendedPlaying) scheduleStartupAutoplayRetry(120);
        if (isChromiumOnlyBrowser()) queueChromiumToggleAudioRepair("play-together-finally");
      } else if (intendedPlaying && isVideoPaused() && !audio.paused) {
        if (isHidden && shouldUseBgControllerRetry()) {
          resumeOnVisible = true;
  ...
  if (qua !== "medium" && hasExternalAudio) {
    ...
    const nudgeStartupReady = () => {
      ...
      else confirmStartupAutoplaySettled();
    };
    ...
    audio.addEventListener("pause", () => {
      ...
      if (startupAutoplaySettlingActive() && intendedPlaying) {
        scheduleStartupAutoplayRetry(100);
        return;
      }
      ...
      if (shouldIgnorePauseAsTransient()) {
        ...
        if (startupAutoplaySettlingActive() && intendedPlaying) scheduleStartupAutoplayRetry(100);
        return;
      }
    ...
    video.on("waiting", () => {
      if (intendedPlaying && !restarting) {
        if (!startupBufferPrimed || startupAutoplayKickInFlight || (startupPhase && !firstPlayCommitted)) {
          if (startupAutoplaySettlingActive() && intendedPlaying) scheduleStartupAutoplayRetry(140);
          return;
        }
        if ((performance.now() - lastPlayKickTs) < 700) return;
