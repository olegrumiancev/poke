...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function doVolumeFade(targetVol, ms = 80) {
    if (!audio) return;
    cancelActiveFade();
    ...
    return new Promise(resolve => {
      const step = () => {
        ...
        // Ease-out curve prevents sharp volume attacks
        const easeT = t * (2 - t); 
  ...
  async function softUnmuteAudio(ms = 120) {
    if (!audio) return;
  ...
  // Critical fix: Time-shifts on the audio element during playback trigger pops. 
  // We duck instantly, move time safely in the void, and ramp back.
  function safeSetAudioTime(t) {
    if (!audio) return;
    try {
      if (isFinite(t) && t >= 0) {
        const timeDiff = Math.abs((audio.currentTime || 0) - t);
        const needsDuck = timeDiff > 0.05 && audio.volume > 0.01 && !audio.paused;
        ...
        if (needsDuck && state.intendedPlaying) {
          // Delay lightly to ensure buffer flushes the gap before unmuting
          setTimeout(() => {
  ...
  function execProgrammaticAudioPause(ms = 320) {
    ...
    try {
      if (!audio.paused) {
        cancelActiveFade();
        audio.volume = 0; // Duck instantly prior to pausing to prevent snap pop
        audio.pause();
      }
    } catch {}
  ...
  async function execProgrammaticAudioPlay(opts = {}) {
    ...
    if (!force && timeSinceLastPlayPause < MIN_PLAY_PAUSE_GAP_MS) {
      if (!audio.paused) softUnmuteAudio(100).catch(()=>{});
      return !audio.paused;
    }
    ...
    try {
      ...
      // Ensure start pop is killed by booting at 0 volume
  ...
  async function softAlignAudioTo(t) {
    ...
    safeSetAudioTime(t);
    if (state.intendedPlaying) softUnmuteAudio(150).catch(()=>{});
  }
    ...
    if (state.intendedPlaying && !state.restarting && !state.seeking && !state.syncing) {
      if (state.strictBufferHold) {
        ...
        if (absDrift > BIG_DRIFT) {
          ...
          if (state.syncConvergenceCount >= 5) resetAudioPlaybackRate();
        }
      }
